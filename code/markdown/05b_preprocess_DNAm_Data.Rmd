---
title: "Run line by line preprocessing of ROSMAP DNA methylation data"
author:
...
get_probe_annotation <- function(array = c("HM450", "EPIC")) {
  if (array == "HM450"){
  ...
  anno_df <- minfi_object %>%
    minfi::getAnnotation() %>%
    data.frame() %>%
    dplyr::select(probe = "Name", "chr", "pos", "strand") %>%
    dplyr::mutate(
      strand_rev = ifelse(.data$strand == "+", "-", .data$strand)
    ) %>%
    dplyr::mutate(
      strand_rev = ifelse(.data$strand == "-", "+", .data$strand_rev)
    )
...
create_summarized_experiment <- function(
  ...
  rowData <- data.frame(
    probe = row.names(assay_data),
    array = array,
    genome = genome
  )
  
  anno_df <- get_probe_annotation(array = array)
  
  rowData <- rowData %>%
    dplyr::left_join(anno_df, by = "probe")
  
  row.names(rowData) <- rowData$probe
...
```{r read_data}
dnam_data <- readRDS(file.path(dataset_dir, "rna_dnam", "dnam_full_data.RDS"))
pheno_df <- dnam_data %>%
...
fail_score <- (rowSums(detP > 0.01) >= 5)
...
## Split by Braak Score

```{r braak_split}
dnam_norm_data <- dnam_data[,dnam_data$braaksc < 3]
dnam_ad_data <- dnam_data[,dnam_data$braaksc > 2]
```
...
saveRDS(
  dnam_data,
  file = file.path(dataset_dir, "rna_dnam", "dnam_processed_data.RDS")
)

saveRDS(
  dnam_norm_data,
  file = file.path(dataset_dir, "rna_dnam", "dnam_processed_normal.RDS")
)

saveRDS(
  dnam_ad_data,
  file = file.path(dataset_dir, "rna_dnam", "dnam_processed_ad.RDS")
)
```
