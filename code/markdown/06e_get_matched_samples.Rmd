...
ad_dnam_df <- readxl::read_excel(
  ...
  as.data.frame() %>%
  dplyr::select(cpg = "CpG", ad_direction = "estimate_direction") %>%
  dplyr::mutate(
...
pdf(
  file.path(
    result_dir, "comparison_aging_and_ad", "comparison-aging-matched-AD-sig.pdf"
  ),
  width = 7,
  height = 6
)
...
pdf(
  file.path(
    result_dir, "comparison_aging_and_ad", "comparison-aging-matched-AD-sig-DMRs.pdf"
  ),
  width = 7,
  height = 6
)
...
write.csv(
  dnam_df,
  file = file.path(
    result_dir, "comparison_aging_and_ad", "Aging_and_Matched_AD_Significant_CpGs.csv"
  )
)
write.csv(
  dmr_df,
  file = file.path(
    result_dir, "comparison_aging_and_ad", "Aging_and_Matched_AD_Significant_DMRs.csv"
  )
)
```
